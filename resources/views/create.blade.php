    ...
    <div class="bg-slate-200 w-full sm:w-1/2 shadow flex flex-col items-center justify-center">
      ...
      <div class="w-full p-3 flex justify-center">
        <form action="{{route('codes.store')}}" method="POST" class="flex flex-col w-full md:w-1/2">
          @csrf
          <label for="quantity" class="my-2">How many codes to generate?</label>
          <input type="number" class="p-2 my-2" min="0" max="10" id="quantity" name="quantity" required>
          <input type="submit" class="py-2 my-2 bg-slate-300 hover:bg-slate-400 cursor-pointer" value="Generate">
        @if(session('success'))
